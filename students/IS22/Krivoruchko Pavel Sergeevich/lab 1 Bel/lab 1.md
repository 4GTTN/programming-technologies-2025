...
В этой части я решал задачу бинарной классификации, где модель должна предсказать один из двух классов (например, 0 или 1).

После подготовки данных, для которой использовалась искусственная выборка с двумя классами, я обучал нейронную сеть с автодифференцированием с использованием алгоритма **градиентного спуска** и определения функции потерь, подходящей для бинарной классификации.

Я использовал новую функцию активации, которая подошла лучше для бинарной классификации.

### Результат обучения:

После того как функция потерь начала стремительно уменьшаться, я проверил результаты. Сеть научилась различать два класса — жёлтые и фиолетовые точки, что подтверждает успешное обучение.
...
Датасет CIFAR-100 содержит 60 000 цветных изображений размером 32×32 пикселя, равномерно распределённых по 100 классам. Я выбрал для классификации три конкретных класса: изображения с яблоками (класс 0), лесами (класс 33) и со львами (класс 43).
